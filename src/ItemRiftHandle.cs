...
{
    /// <summary>
    /// The handle. Extends ARL's item class so shapes, textures and names keep coming from
    /// the JSON maps; what it adds is deciding a handle's attributes from what went into
    /// the grid.
    ///
    /// WHY EVERY HANDLE RECIPE HAS A BARE OUTPUT. ARL matches a recipe to a handbook page
    /// by EXACT stack equality. A recipe declaring SOME attributes - `tier`, or a grip and
    /// its material - equals no page at all, so handles showed no "Created by" however they
    /// were written. A recipe declaring NOTHING is treated as generic and does show, which
    /// is why the sword's assembly recipe always appeared. Moving every declaration in here
    /// buys the handbook entry back and removes the last hard-coded assumption about which
    /// recipe produces what.
    ///
    /// It also makes wrapping non-destructive: re-gripping a fitted handle keeps its guard,
    /// because nothing is declared that could overwrite it.
    /// </summary>
    public class ItemRiftHandle : AttributeRenderingLibrary.ItemShapeTexturesFromAttributes
    {
